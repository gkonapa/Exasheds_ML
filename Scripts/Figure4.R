...
DL_data_diff <- DL_data$HYBRID1 - DL_data$HYBRID2
...
ggplot(data=pp_DL,aes(x=`_x_`,y=`_yhat_`))+geom_line()+
  facet_wrap(~`_vname_`,scales = "free_x",nrow = 2)+theme_bw()+theme(panel.grid.major = element_blank(),
                                                                     panel.grid.minor = element_blank(),text = element_text(size=15))+labs(y=expression("Average effect on"~Delta~"NSE"[Hybrid1-Hybrid2]),x="")


pp_DL$vars=as.character(pp_DL$`_vname_`)
unique(pp_DL$vars)
b<-ggplot(data=pp_DL,aes(x=`_x_`,y=`_yhat_`))+geom_line()+
  facet_wrap(~`_vname_`,scales = "free_x",nrow = 2)+theme_bw()+theme(panel.grid.major = element_blank(),
                                                                panel.grid.minor = element_blank(),text = element_text(size=15))+labs(y=expression("Average effect on"~Delta~"NSE"[Hybrid1-Hybrid2]),x="")


png("./fig5.png",width=15, height=6, units="in",res=600)
b
dev.off()

png("./figS6.png",width=10, height=16, units="in",res=600)
a
dev.off()