...
DL_data_diff <- DL_data$HYBRID1 - DL_data$LSTM
...
topx=as.character(vimp_DL[order(vimp_DL$median,decreasing = TRUE),][1:15,1])
print(topx)
remove = c("high_q_dur","soil_depth_pelletier","high_prec_freq","hfd_mean","low_q_dur")
topx <- topx[!topx %in% remove]
pp_DL<-conditional_dependency(explain_DL, variables =  topx)
pp_DL$vars=as.character(pp_DL$`_vname_`)
unique(pp_DL$vars)
pp_DL$vars[pp_DL$vars=="root_depth_99"]="99 percentile\nroot depth (m)"
pp_DL$vars[pp_DL$vars=="p_mean"]="Precipitation (mm/day)"
pp_DL$vars[pp_DL$vars=="aridity"]="Aridity"
...
pp_DL$vars[pp_DL$vars=="elev_mean"]=NA
#pp_DL$vars[pp_DL$vars=="elev_mean"]="Elevation (m)"
pp_DL$vars[pp_DL$vars=="high_q_dur"]="High flow\nduration (days)"
...
pp_DL$vars[pp_DL$vars=="runoff_ratio"]=NA
#pp_DL$vars[pp_DL$vars=="runoff_ratio"]="Runoff ratio"
pp_DL$vars[pp_DL$vars=="hfd_mean"]="Half flow\ndate (day of year)"
#pp_DL$vars[pp_DL$vars=="frac_forest"]="Forest area fraction"
pp_DL$vars[pp_DL$vars=="frac_forest"]=NA
#pp_DL$vars[pp_DL$vars=="gvf_max"]="Maximum monthly\nvegetation "
pp_DL$vars[pp_DL$vars=="gvf_max"]=NA

orders = c("Snow fraction","Elevation (m)","Precipitation (mm/day)","aridity","Low flow\nDuration (days)",
           "Low flow\nfrequency (days/year)","Baseflow index","Half flow\ndate (day of year)")

pp_DL$`_vname_` = as.factor(pp_DL$vars)
drops <- c("vars","_ids_","_label_")
orders = c("99 percentile\nroot depth (m)","Precipitation (mm/day)","Aridity","Snow fraction","Streamflow (mm/day)","Q95 (mm/day)")
...
b<-ggplot(data=moddata,aes(x=x,y=y))+geom_line()+
  facet_wrap(~type,scales = "free_x",nrow = 2)+theme_bw()+theme(panel.grid.major = element_blank(),
                                                        panel.grid.minor = element_blank(),text = element_text(size=15))+labs(y=expression("Average effect on"~Delta~"NSE"[Hybrid1-LSTM]),x="")

png("./fig4.png",width=10, height=6, units="in",res=600)
b
dev.off()

png("./figS3.png",width=10, height=16, units="in",res=600)
a
dev.off()